...
2. **Rotular (Labeling):**
   Vamos usar o **labelImg** para marcar onde estão as peças nas fotos.

   ```bash
   labelImg dataset classes.txt
   ```

   - Pressione **'w'** para desenhar a caixa na peça.
   - Digite o nome **`peca`**.
   - Salve com **Ctrl+S** (verifique se está no formato **YOLO** na barra lateral).
   - Vá para a próxima imagem com **'d'**.
